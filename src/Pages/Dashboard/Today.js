...
function Today() {
  ...
  .catch(error => {
    ...
    function deleteTask(e) {
        ...
        axios.post('http://localhost:8000/deleteTask', inputData)
  .then(response => {
    console.log('PUT request successful:', response);
    saveUserData(response.data)
    setModal(null)
  ...
  return (
    ...
    <div className={styles.todayWrapper}>
      ...
      <div className={styles.listWrapper}> {tasksToday && tasksToday.map((item, index) => <TodayListItem props={item} openModal={openModal}>item {index}</TodayListItem>)}</div>
    </div>
    {modal && <div className={styles.today_task_modal} >
        <div className={styles.modal_title}>Task:
